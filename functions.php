...
function assetwise_scripts()
{
  ...
  if (get_page_template_slug(get_the_ID()) === 'pages/page-main-project.php') {
    wp_enqueue_style('Main Page Styles', get_template_directory_uri() . '/styles/page-main-project.css', '', '1.0.0');
  } elseif (get_page_template_slug(get_the_ID()) === 'pages/page-main-project-kave.php') {
    wp_enqueue_style('Main Page Styles', get_template_directory_uri() . '/styles/page-main-project-kave.css', '', '1.0.0');
  } elseif (get_page_template_slug(get_the_ID()) === 'pages/page-campaign.php') {
    wp_enqueue_style('Campaign Page Styles', get_template_directory_uri() . '/styles/campaign.css', '', '1.0.4');
  }
  if (get_page_template_slug(get_the_ID()) === 'pages/page-pre-approved.php') {
    wp_enqueue_style('Pre-Approved Style', get_template_directory_uri() . '/styles/pre-approved_page.css');
    wp_enqueue_script('Pre-Approved Script', get_template_directory_uri() . '/js/per-approved.js', '', '', true);
  }
...
function project_Assets() {
  ...
  if(is_singular('condominium') || is_singular('house')) {
  ...
  if (is_singular('condominium')) {
    ...
    // Kave AVA
    if (get_the_ID() == 36600) {
      wp_enqueue_style('Kave AVA Styles', get_template_directory_uri() . '/styles/kave-ava/style.css');
      //wp_enqueue_script('BS-Kave AVA', 'https://cdn.jsdelivr.net/npm/bootstrap@4.6.0/dist/js/bootstrap.bundle.min.js', '', '', true);
    }
  }
...
add_action( 'wp_ajax_get_project_register_list', 'get_project_register_list' );

function get_project_register_list() {
  $db_key = $_POST['key'];

  $query = "SELECT CONCAT(c.name, ' ', c.surname) AS name_register,
            c.*
            FROM `register_$db_key` c
            ORDER BY created_at DESC;";

	if($_SERVER['SERVER_NAME'] === 'kavecondo.com') {
		$assetwise_db = new wpdb(
			'kavecondo_db',
			'pghc5e-U_c',
			'kavecondo_register',
			'localhost'
  ...
  $rows = $assetwise_db->get_results($query);

	foreach ($rows as $row) {
	  $addi = json_decode($row->additional);
    $priceRage = '';
    $roomType = '';
	  if($addi !== null) {
      $priceRage = $addi->priceRage;
      $roomType = $addi->roomType;
    } else {
      $priceRage = '-';
      $roomType = '-';
    }
	  $row->price_range = $priceRage;
	  $row->room_type = $roomType;
  }

  $obj2 = json_encode($rows);
...
function galleries () {
  ob_start();
  include_with_variables(get_stylesheet_directory() . '/template-parts/shortcode/gallery-template.php');
  return ob_get_clean();
}
...
function ava_concepts () {
  ob_start();
  include_with_variables(get_stylesheet_directory().'/template-parts/shortcode/kave-ava/ava_concept.php');
  return ob_get_clean();
}

function ava_floorPlan() {
  ob_start();
  include_with_variables(get_stylesheet_directory().'/template-parts/shortcode/kave-ava/floor-plan-ava.php');
  return ob_get_clean();
}

function unit_plans_single() {
  ob_start();
  include_with_variables(get_stylesheet_directory().'/template-parts/shortcode/kave-ava/unit-plan-single.php');
  return ob_get_clean();
}

function kave_ava_locations() {
  ob_start();
  include_with_variables(get_stylesheet_directory().'/template-parts/shortcode/kave-ava-location.php');
  return ob_get_clean();
}
...
function ava_icons() {
  ob_start();
  include_with_variables(get_stylesheet_directory() . '/template-parts/shortcode/kave-ava/kave-ava_icons.php');
  return ob_get_clean();
}

function kave_colony_floorplan() {
  ob_start();
  include_with_variables(get_stylesheet_directory() . '/shortcode/modules/colony-floor-plan.php');
  return ob_get_clean();
}
...
function wpml_custom_lang_listed() {
  check_active_languages();
}

// Undone Module - Joe 21Jun2022
function images_gallery($attr) {
  ob_start();
  //var_dump($attr['field']);
  //$images = get_field($attr['field']);
  $images = 'foo';
  //include get_stylesheet_directory() . '/template-parts/shortcode/gallery-slider.php';
  include_with_variables(get_stylesheet_directory() . '/template-parts/shortcode/gallery-slider.php', $attr['field']);
  return ob_get_clean();
}

function check_active_languages(){
  ...
  if ( !empty( $languages ) ) {
    foreach( $languages as $l ) {
      ...
      //if ( !$l['active'] ) echo esc_attr( $l['language_code'] .' ' );
    }
  }
}
...
add_shortcode('wpml_custom_langs', 'wpml_custom_lang_listed');

add_shortcode('kave-ava-locations', 'kave_ava_locations');
add_shortcode('kave-ava-concepts', 'ava_concepts');
add_shortcode('kave-ava-floorPlan', 'ava_floorPlan');
add_shortcode('kave-ava-icons', 'ava_icons');
add_shortcode('unit-plan-single', 'unit_plans_single');

add_shortcode('kave-colony-floorplan', 'kave_colony_floorplan');

add_shortcode('swipe-gallery', 'images_gallery');
